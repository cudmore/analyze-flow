...
imageFrames = [];
disp('   imimportTif() from Kim et al');
for index = 1:256
    try
    ...
    fprintf('   image: %d, ', index);
    if mod(index,10) == 0
        fprintf('\n')
    end
    if index == 256
        disp('   Warning: Arbitrary 256 frame TIF limit reached. Increase if necessary')
    end
end
fprintf('\n');
...
if length(imageFramesSize) == 2
    % abb
    imageLines = zeros(imageFramesSize(2), imageFramesSize(1));
    index = 1;
...
disp('   imimportTif() Multiframe import complete');
