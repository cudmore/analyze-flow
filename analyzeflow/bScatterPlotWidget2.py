...
class myStatListWidget(QtWidgets.QWidget):
    ...
    def getCurrentStat(self):
        ...
        try:
            ...
            #logger.warning(f'Did not find humanStat:"{humanStat}"')
            humanStat = humanStat
            stat = humanStat
...
class myMplCanvas(QtWidgets.QFrame):
    ...
    def on_pick_event2(self, event):
        ...
        #
        logger.info(f'  -->> signalSelectSquare.emit() plotNumber:{self.plotNumber} stateDict:')
    ...
    def getAnnotation(self, ind):
        """
        ind is into the plot (different from master df index)
        """
...
class bScatterPlotMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, path,
        ...
        self.dataType = 'All Spikes'  # 'File Mean' # (Raw, File Mean)
    ...
    def buildUI(self, interfaceDefaults):
        ...
        self.dataTypeDropdown.setCurrentIndex(0)  # 'All Spikes'
    ...
    def getState(self):
        ...
        if self.dataType == 'All Spikes':
            ...
            logger.error(f'  error in dataType:{self.dataType}')
    ...
    def on_button_click(self, name):
        ...
        if name == 'Replot':
            ...
            if self.sortOrder is None:
                self.mySetStatusBar(f'Sort order is None, no sorting')
            else:
    ...
    def getMeanDf(self, xStat, yStat, verbose=False):
        ...
        if yIsCategorical or groupByNone:
            ...
            # declan jan 2023, try and get t-test b/w each group
            from itertools import combinations
            from scipy.stats import ttest_ind

            logger.info('=== ADDING TTEST')
            _groupByColumnName = self.groupByColumnName
            grps = self.masterDf[_groupByColumnName].unique()
            combs = combinations(grps, 2)
            logger.info(f'  _groupByColumnName: {_groupByColumnName}')
            logger.info(f'  grps: {grps}')
            logger.info(f'  combs: {combs}')
            # TODO: declan is using 'one-taild'
            # alternative{‘two-sided’, ‘less’, ‘greater’}, optional
            # With symmetric distributions one-sided p-value is just half of the two-sided pvalue
            ttests = {
                f'{c1}_{c2}': ttest_ind(
                    self.masterDf.loc[self.masterDf[_groupByColumnName] == c1, yStat], 
                    self.masterDf.loc[self.masterDf[_groupByColumnName] == c2, yStat]
                ) for c1, c2 in combs
            }
            print(ttests)
            print('')
        ...
        for i, analysisname in enumerate(fileNameList):
            ...
            if len(tmpDf) == 0:
                logger.error(f'  ERROR: got 0 length for analysisname: {analysisname}')
                continue
            ...
            for catName in self.masterCatColumns:
                ...
                if numUnique == 1:
                    ...
                    logger.warning(f'catName:{catName} has {numUnique} unique values but expect only 1')
                    pass
        ...
        if self.sortOrder is not None:
            logger.info(f'  sorting by self.sortOrder:{self.sortOrder}')
            try:
                meanDf = meanDf.sort_values(self.sortOrder)
                if self.xDf is not None:
    ...
    def slotSelectSquare(self, plotNumber, stateDict):
        ...
        logger.info('  todo: fill in all interface using stateDict')
...
def testOligo():
    ...
    '''
    categoricalList, hueTypes, analysisName,
    sortOrder=None,
    statListDict=None,
    interfaceDefaults=None,
    masterDf=None,
