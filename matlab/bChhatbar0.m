function [angle,utheta,uvar] = bChhatbar0(inputimg,showimg,saveimg,delx,delt,hi,lineskip,xrange)

%
% Robert Cudmore modified version of the original
%
...
curimtitle = {'edm','vdm','Chhatbar sob'}; curcolor = {'b','g','k','r','c','m'};
...
% abb i had this but reverting to original chhatbar plot
%         figChattbar = figure;
...
%         
%         subplot(3,1,3);
%         plot(angle(:,3,ii)*delt,angle(:,9,ii),['-o' curcolor{ii} ]);
%         xlabel('time (ms)');
%         ylabel('v (mm/s)');
        ...
        for jj=1:segn
            ...
            line(angle(jj,3,ii)-[-yp yp], xrange(1)+wi/2+[-xp xp], 'Color','black','LineWidth',anglineth,'EraseMode','xor');
        end
        hold off;
        if ii==1
        ...
        subplot(3,1,2)
        plot(angle(:,3,ii)*delt,angle(:,1,ii),['-o' curcolor{ii} ]);
        hold on;
        if ii==imgsegsz(3)
            legend(curimtitle{ii}); 
